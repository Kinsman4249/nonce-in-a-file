...
var OUTPUT_CSS = [
  ...
  '.sign .confirm{display:block;font-size:13px;color:inherit;cursor:pointer}',
...
var OUTPUT_JS = [
  ...
  '    // own: it shows amber (integrity OK) until the recipient confirms the',
  '    // 8-byte key fingerprint with the sender out-of-band and ticks the box.',
  ...
  '          sigMsg.textContent="Signed and verified: from "+(payload.sign.label||"this sender")+" (key "+fp+"). Fingerprint confirmed with the sender out-of-band; file not altered since signing.";',
  '        }else{',
  '          sigEl.className="sign info";',
  '          sigMsg.textContent="Integrity OK - not altered since signing, but sender authenticating has NOT yet been confirmed. Verify the key fingerprint "+fp+" with the sender out-of-band, then tick the box below.";',
  '        }',
  '      }',
  '      confirmBox.addEventListener("change",paint);',
...
function makeOutput(o, payload) {
  ...
  var signBadge = payload.sign
    ? [
        ...
        '<input type="checkbox" id="signConfirm">',
        ' I verified this key fingerprint with the sender out-of-band',
        '</label>',
        '</div>'
      ].join('')
    : '';
